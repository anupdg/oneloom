...
function initViewer() {
  ...
  if (typeof viewer.onApiUserStateChanged === 'function') {
    viewer.onApiUserStateChanged(function(key, value) {
      console.log("API user state changed:", key, value);
      ...
      if (key.startsWith("MaterialPicker:")) {
        ...
        console.log("Material changed via Shapespark picker:", material, "to", value);
        
        // Send the updated selection to the parent window
        sendSelectionUpdate();
      }
    });
    console.log("Set up Shapespark API state change listener");
  } else {
    console.warn("viewer.onApiUserStateChanged is not available");
  }
